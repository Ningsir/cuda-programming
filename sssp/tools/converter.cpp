...
int main(int argc, char **argv)
{
    ...
    if (GetFileExtension(input) == "wel")
    {
        ...
        nodePointer[num_nodes] = counter;
        uint *outDegreeCounter = new uint[num_nodes];
        memset(outDegreeCounter, 0, sizeof(uint) * num_nodes);
        uint location;
        for (uint i = 0; i < num_edges; i++)
        {
        ...
        outfile.write((char *)nodePointer, sizeof(unsigned int) * (num_nodes + 1));
